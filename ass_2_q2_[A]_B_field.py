...
save_path="./B_field_lines_vx={vel:.2f} for vdot =0.3 z.mp4".format(vel=vx)
...
#Radiation Magnetic Field per unit charge times c, in Bi(ct,x,y,z) axes
...
Bx=Ex*y/R - Ey*(zb/R+vz)
By=Ex*(zb/R+vz) - Ez*(xb/R+vx)
Bz=Ey*(xb/R+vx) - Ex*y/R
...
def update(i):
    ...
    k=((gamma*zb)**2 + y**2 + (gamma*xb)**2)**(1/2)
    R=gamma**2*(vx*xb + vz*zb) + gamma*k
    #Radiation Magnetic Field per unit charge times c, in Ei(ct,x,y,z) axes
    Ex = gamma**3 * (v_dot*xb*zb + v_dot*xb*vz*R)/k**3
    Ey = gamma**3 * (v_dot*y*zb + v_dot*y*vz*R)/k**3
    Ez= -gamma**3 * (v_dot*xb**2 + v_dot*y**2 + v_dot*xb*R)/k**3
    ...
    ax1.clear()
    ax1.set_title('Radiation Magnetic Field for vdot =0.3 z v={vel:.2f}c'.format(vel=vx))
    ax1.set_xlim(-xy_max, xy_max)
    ax1.set_ylim(-xy_max, xy_max)
    ax1.set_zlim(-xy_max, xy_max)
    ...
    ax1.set_zlabel("z")
    ax1.grid()
    ax1.quiver(x, y, z, Bx, By, Bz, color='r',length=arrow_length)
